...
import { JharkhandTourismUI } from '../components2_bank/tour/JharkhandTourismUI';
...
export default function JharkhandPage() {
    ...
    const handleConnect = useCallback(async () => {
        ...
        try {
            ...
            const url = `${TOKEN_ENDPOINT}?name=${userId}&agent=tour`;
            ...
            <JharkhandTourismUI>
                ...
                {token ? (
                    ...
                    <div className="fixed bottom-24 md:bottom-8 right-4 md:right-8 z-50 flex flex-col items-end gap-4">
                        ...
                        {/* Floating Trigger Button (Matches the Star button in the image) */}
                        {!isCardOpen && (
                            <button
                                onClick={() => setIsCardOpen(true)}
                                disabled={connecting}
                                className="pointer-events-auto group relative w-16 h-16 rounded-full bg-gradient-to-br from-[#f4a024] to-[#d68515] shadow-[0_4px_20px_rgba(244,160,36,0.4)] flex items-center justify-center transition-transform hover:scale-110 active:scale-95 disabled:scale-100 disabled:opacity-80 border-4 border-white/20"
                            >
                                {/* Tooltip */}
                                <div className="absolute right-full mr-4 bg-black/80 backdrop-blur text-[#f4a024] px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-[#f4a024]/20">
                                    Ask Assistant
                                </div>

                                <div className="relative z-10">
                                    {/* Using Compass/Star icon to match the tourism image style */}
                                    <Compass size={32} className="text-white animate-pulse" />
                                </div>

                                {/* Ping animation */}
                                <div className="absolute inset-0 rounded-full bg-[#f4a024] opacity-50 animate-ping"></div>
                            </button>
                        )}
                    </div>
                )}
